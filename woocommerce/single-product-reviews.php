 ...
 * @version     3.5.0
 * @karino-note UPDATE UP TO Commits on Aug 29, 2018 WOO V 3.5.0 ===> IN TIVA V5.5.2
 */
if (!defined('ABSPATH')) {
    ...
    <div id="comments">
        <h2 class="woocommerce-Reviews-title"><?php
            if (get_option('woocommerce_enable_review_rating') === 'yes' && ($count = $product->get_review_count())) {
                $new_count = tiva_change_number($count);
                /* translators: 1: reviews count 2: product name */
                printf(esc_html(_n('%1$s review for %2$s', '%1$s reviews for %2$s', $new_count, 'woocommerce')), esc_html($new_count), '<span>' . get_the_title() . '</span>');
            } else {
